...
export default function ComposerStepUpload({ channels, onNext }) {
  ...
  const parseFile = useCallback(async (file) => {
    setError("");
    if (file.size > MAX_BYTES) { setError("File too large (max 500 KB)."); return; }
    const ext = "." + file.name.split(".").pop().toLowerCase();
    if (!ACCEPTED.includes(ext)) { setError("Only .md, .txt, and .pdf files are supported."); return; }
    ...
    <div className="space-y-5">
      <div>
        ...
        <div
          ...
          onDrop={handleDrop}
          className={`relative border-2 border-dashed rounded-xl p-6 text-center transition-all ${dragging ? "border-indigo-400 bg-indigo-50" : "border-slate-200 hover:border-slate-300 bg-slate-50"}`}
        >
          {fileName ? (
            <div className="flex items-center justify-center gap-3">
              <FileText className="w-5 h-5 text-indigo-500" />
              <span className="text-sm font-medium text-slate-700">{fileName}</span>
              <button onClick={() => { setFileName(""); setRawContent(""); setTitle(""); }} className="p-1 rounded-full hover:bg-slate-200 text-slate-400 hover:text-slate-600 transition-colors" aria-label="Remove file">
                <X className="w-4 h-4" />
              </button>
            </div>
          ) : (
      ...
      <div>
        ...
        <Input id="title" placeholder="e.g. TOP 100 Aerospace Season 4 Launch" value={title} onChange={e => setTitle(e.target.value)} />
      </div>
      ...
      <div>
        ...
        <Input id="objective" placeholder="e.g. Grow LinkedIn followers by 20% this quarter" value={objective} onChange={e => setObjective(e.target.value)} />
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label htmlFor="platform" className="mb-2 block">Platform</Label>
          <select id="platform" value={selectedPlatform} onChange={e => setSelectedPlatform(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 min-h-[44px] bg-white">
        ...
        <div>
          <Label htmlFor="post_count" className="mb-2 block"># of Posts <span className="text-xs text-slate-400 font-normal">(1–12)</span></Label>
          <input id="post_count" type="number" min={1} max={12} value={postCount} onChange={e => setPostCount(Math.min(12, Math.max(1, parseInt(e.target.value) || 1)))} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 min-h-[44px]" />
        </div>
      </div>
      ...
      <Button onClick={() => onNext({ rawContent, title, objective, platform: selectedPlatform, postCount, selectedChannelIds })} disabled={!canProceed} className="w-full gap-2 bg-indigo-600 hover:bg-indigo-700 min-h-[44px]">
