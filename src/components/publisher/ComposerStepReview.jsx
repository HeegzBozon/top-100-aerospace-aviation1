...
import {
  ChevronLeft, ChevronRight, RefreshCw, Check, Send,
  Wand2, AlertTriangle, Clock
} from "lucide-react";
...
export default function ComposerStepReview({
  posts: initialPosts,
  context,
  userEmail,
  onBack,
  ...
  return (
    <div className="space-y-4">
          ...
          {posts.map((p, i) => (
            <div
              ...
              className={`h-1.5 flex-1 rounded-full cursor-pointer transition-all ${
                p.approved ? "bg-emerald-400" : i === currentIdx ? "bg-indigo-500" : "bg-slate-200"
              }`}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <Badge className={`text-xs ${JJJH_COLORS[currentPost.jjjh] || ""}`}>
          {currentPost.jjjh}
        </Badge>
        <Badge className={`text-xs ${AIDA_COLORS[currentPost.aida] || ""}`}>
          {currentPost.aida}
        </Badge>
        ...
        <span className={`ml-auto text-xs font-mono ${isOverLimit ? "text-red-500 font-bold" : "text-slate-400"}`}>
          {currentPost.content.length} / {charLimit}
          {isOverLimit && <span className="ml-1">⚠ over limit</span>}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          aria-label="Regenerate post with AI"
        >
        ...
        <Button
          variant={currentPost.approved ? "default" : "outline"}
          size="sm"
      ...
      <div className="flex items-center justify-between border-t border-slate-100 pt-3">
        ...
        <Button
          variant="ghost"
          size="sm"
          onClick={() => setCurrentIdx(i => Math.min(posts.length - 1, i + 1))}
      ...
      <div className="flex items-center gap-3 border-t border-slate-100 pt-4">
        ...
        <Button
          ...
          disabled={approvedCount === 0 || saveMutation.isPending}
          className="flex-1 gap-2 bg-indigo-600 hover:bg-indigo-700 min-h-[44px]"
        >
          <Send className="w-4 h-4" />
          {saveMutation.isPending
            ? "Saving…"
            : `Save ${approvedCount} Post${approvedCount !== 1 ? "s" : ""} as Drafts`}
        </Button>
      </div>

      {approvedCount === 0 && (
        <p className="text-xs text-slate-400 text-center">Approve at least one post to save.</p>
      )}
    </div>
  );
}