...
export default function StoriesBar({ user, accent }) {
    ...
    <>
      <section className="rounded-2xl overflow-hidden" style={{ background: B.cream, border: `1px solid ${B.border}` }}>
        ...
        <div className="px-5 py-4 flex gap-4 overflow-x-auto scrollbar-hide">
          <button onClick={() => (myGroupIdx >= 0 ? setViewerIdx(myGroupIdx) : setCreating(true))} className="flex flex-col items-center gap-1 shrink-0">
          ...
          {others.map((g) => {
            ...
            return (
              <button key={g.author.email} onClick={() => setViewerIdx(realIdx)} className="flex flex-col items-center gap-1 shrink-0">
      ...
      {viewerIdx !== null && groups[viewerIdx] && (
        <StoryViewer groups={groups} startGroupIdx={viewerIdx} onClose={() => setViewerIdx(null)} viewerEmail={user?.email} />
      )}
      {creating && (
        <StoryCreate user={user} accent={accent} onCreate={create} onClose={() => setCreating(false)} />
      )}
    </>
  );
}