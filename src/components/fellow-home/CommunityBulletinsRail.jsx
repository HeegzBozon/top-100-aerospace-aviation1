...
export default function CommunityBulletinsRail({ user, accent, bare }) {
  ...
  if (bare) return <>{inner}{modal}</>;
  return (
    <>
      <RailBlock title="Community Bulletins" accent={accent}>{inner}</RailBlock>
      {modal}
    </>
  );
}