...
export default function StatusPicker({ statusKey, accent, saving, onChange, compact, centered }) {
  ...
  // Click-outside dismiss so the popover behaves like a real menu.
  useEffect(() => {
    if (!open) return;
  ...
  const menuClass = centered ? 'left-1/2 -translate-x-1/2 w-full max-w-xs' : 'right-0';

  return (
    <div ref={ref} className={`relative ${centered ? 'flex flex-col items-center w-full' : 'inline-block'}`}>
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div
          className={`absolute z-50 mt-1 grid grid-cols-1 gap-0.5 rounded-lg p-1 shadow-lg ${menuClass}`}
          style={{ background: '#fff', border: `1px solid ${B.border}`, top: '100%' }}
        >
          {FELLOW_STATUSES.map((s) => (
            <button
              key={s.key}
              onClick={() => { onChange(s.key === statusKey ? '' : s.key); setOpen(false); }}
