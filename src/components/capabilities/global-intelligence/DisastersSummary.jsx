...
export function DisastersSummary() {
  const { data, isLoading } = useNaturalDisasters();
  const events = data?.events ?? [];
  const critical = events.filter(e => e.severity === 'critical').length;
  const quakes = events.filter(e => e.type === 'earthquake').length;
  const topMag = events
    .filter(e => e.magnitude != null)
    .reduce((max, e) => Math.max(max, e.magnitude), 0);

  if (isLoading) return <SummaryLoader />;
  if (!events.length) return <SummaryEmpty label="No disaster data" />;

  return (
    <div className="h-full flex flex-col gap-1 p-1">
      <SummaryStat value={events.length} label="EVENTS" color="text-orange-400" />
      <SummaryStat value={critical} label="CRITICAL" color="text-red-400" />
      <SummaryStat value={topMag > 0 ? `M${topMag.toFixed(1)}` : quakes} label={topMag > 0 ? 'TOP MAG' : 'QUAKES'} color="text-orange-300" />
    </div>
  );
}
