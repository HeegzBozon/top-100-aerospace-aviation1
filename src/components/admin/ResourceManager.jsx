...
export default function ResourceManager() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Booking Links Quick Section */}
      {(() => {
        const bookingResources = resources.filter(r => r.tags?.includes('booking'));
        if (bookingResources.length === 0) return null;
        return (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <Calendar className="w-4 h-4 text-blue-600" />
              <h3 className="text-sm font-bold text-blue-800">Booking Links</h3>
            </div>
            <div className="flex flex-wrap gap-2">
              {bookingResources.map(r => {
                const url = r.reference_urls?.[0];
                return (
                  <div key={r.id} className="flex items-center gap-2 bg-white border border-blue-200 rounded-lg px-3 py-2 shadow-sm">
                    <div>
                      <p className="text-xs font-semibold text-slate-800">{r.display_name}</p>
                      {url && (
                    ...
                    <button onClick={() => handleEdit(r)} className="p-1 hover:bg-slate-100 rounded ml-1">
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            {/* Booking URL quick field */}
            {form.tags?.includes('booking') && (
              <div className="space-y-1.5 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <Label className="text-blue-800 font-semibold flex items-center gap-1.5">
                  <Calendar className="w-4 h-4" /> Booking URL
                </Label>
                <Input
                  placeholder="https://calendly.com/yourname or https://cal.com/yourname"
                  value={form.reference_urls?.[0] || ''}
                  onChange={e => {
                    const val = e.target.value;
                    setForm(f => ({ ...f, reference_urls: val ? [val, ...(f.reference_urls || []).slice(1)] : (f.reference_urls || []).slice(1) }));
                  }}
                  className="bg-white"
                />
              </div>
