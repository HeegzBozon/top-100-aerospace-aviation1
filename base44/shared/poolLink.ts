...
function keysOverlap(a, b) {
  ...
  return a.emails.some((e) => b.emails.includes(e));
}
...
// Find an existing live master for a set of identity keys. Slug wins over email.
export function findPoolMatch(pool, keys, excludeId) {
  const live = pool.filter((n) => n.id !== excludeId && !isMerged(n));
...
// Approve a NominationIntake into the pool: link to an existing master or create one.
export async function linkIntakeToPool(sr, intake, seasonId, pool) {
  ...
  const { match, matched_on } = findPoolMatch(pool, intakeKeys(intake));
  ...
  if (match) {
    ...
    const created = await sr.entities.Nominee.create({
      ...
      nomination_count: 1,
      raw_nomination_data: { is_master: true, season_participation: [{ nominee_id: null, season_id: seasonId, status: 'pending' }], merged_nominee_ids: [] },
    });
    pool.push(created);
  ...
  const note = match ? `Linked to pool master (${matched_on} match).` : 'Created new pool master.';
  await sr.entities.NominationIntake.update(intake.id, {
    status: 'approved',
    nominee_id: nomineeId,
    admin_notes: `${intake.admin_notes ? intake.admin_notes + '\n' : ''}${note}`,
  });
  return { status: match ? 'linked' : 'created', matched_on, nominee_id: nomineeId, nominee_name: nomineeName };
}
...
export async function approveNomineeInPool(sr, nominee, pool, intakes, force) {
  ...
  const { match, matched_on } = findPoolMatch(pool, keys, nominee.id);
...
export function scanSeasonDuplicates(pool, seasonId) {
  ...
  live.forEach((n, i) => {
    ...
    const all = [...(k.slug ? ['s:' + k.slug] : []), ...k.emails.map((e) => 'e:' + e)];
  ...
  for (const members of groups.values()) {
    ...
    if (inSeason.length >= 2) {
      const slugs = uniq(inSeason.map((n) => nomineeKeys(n).slug));
      mergeable.push({ matched_on: slugs.length === 1 && slugs[0] ? 'linkedin' : 'email', members: inSeason.map(slimNominee) });
...
export function findOrphans(pool, intakes, seasonId) {
  const linked = new Set(intakes.filter((i) => i.nominee_id).map((i) => i.nominee_id));
  const unlinked_intakes = intakes.filter((i) => i.status === 'approved' && !i.nominee_id).map(slimIntake);
