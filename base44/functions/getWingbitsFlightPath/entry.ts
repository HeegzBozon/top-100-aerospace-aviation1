...
// GET /v1/flights/{icao24}/path — complete flight trajectory.
// Accepts: { icao24 } in request body (POST) or ?icao24= query param (GET).
// Returns: { flight: { id, name, path: [{latitude, longitude, altitude, timestamp}] } }
// Cache TTL: 60 minutes — completed paths change infrequently within a mission window.
...
Deno.serve(async (req) => {
  try {
    ...
    const res = await fetch(`https://customer-api.wingbits.com/v1/flights/${key}/path`, {
    ...
    // { flight: { id, name, path: [{latitude, longitude, altitude, timestamp}] } }
