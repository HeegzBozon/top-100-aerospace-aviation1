...
// GET /v1/flights/{icao24} — full live telemetry for one aircraft.
// Accepts: { icao24 } in request body (POST) or ?icao24= query param (GET).
// Returns: { flight, lastPosition, message } per Wingbits spec.
// Cache TTL: 10 seconds — live positional data is highly time-sensitive.
...
Deno.serve(async (req) => {
  try {
    ...
    const res = await fetch(`https://customer-api.wingbits.com/v1/flights/${key}`, {
    ...
    // { flight: Flight|null, lastPosition: {la,lo,alt,timestamp}|null, message: string|null }
